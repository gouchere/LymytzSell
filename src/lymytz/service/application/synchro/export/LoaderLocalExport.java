...
public class LoaderLocalExport extends Task<ObservableList<ObservableList>> {
    ...
    public ObservableList<ObservableList> requeteLibre(PreparedStatement st) {
        ...
        try (ResultSet rs = st.executeQuery()) {
            ...
            while (rs.next()) {
                ...
                idx = 0;
                line = new LymytzData(new EntityColumn[colonnes.size() + 4]);
                row.add(String.valueOf(progress + 1));
                for (int i = 0; i < (colonnes.size() + 4); i++) {
                    switch (i) {
                        case 0:
                            line.getValue()[i] = fillColumn(rs.getMetaData().getColumnTypeName(i + 1), new EntityColumn("id_ds", "id_ds", null, Long.TYPE), rs, (i + 1));
                            break;
                        case 1:
                            ...
                            break;
                        case 2:
                            line.getValue()[i] = fillColumn(rs.getMetaData().getColumnTypeName(i + 1), new EntityColumn("id_listen", "id_listen", null, Long.TYPE), rs, (i + 1));
                            break;
                        case 3:
                            line.getValue()[i] = fillColumn(rs.getMetaData().getColumnTypeName(i + 1), new EntityColumn("action_name", "Action", null, String.class), rs, (i + 1));
                            break;
                        default:
                    ...
                    row.add(fillColumn(rs.getMetaData().getColumnTypeName(i + 1), rs, (i + 1)));
                }
                listData.add(line);
                re.add(row);
                progress++;
    ...
    protected ObservableList<ObservableList> call() throws Exception {
        return requeteLibre(dao.buildQueryToSynchronise(table, colonnes, "l.name_table", table));
    }
